...
<script type="text/javascript">
	$().ready(function() {
		...
		$("tr").dblclick(function() {
		 ...
		 $("#emp .upda").find("input").blur(function(){
				 var array= new Array;
				...
				if(array!=""){
				location.href="scoreServlet2?who=update2All&array="+array1;
				}else{
					alert("请选中数据");
				}
...
<body>
	...
	<div class="box2">
		<ul class="pagination" style="float: right;">
			<li><a id="pre"
				href="scoreServlet2?page=${p.nowPage-1}&name=${c.employee.name }&deptId=${c.employee.dept.id}&proId=${c.project.id}&value=${c.value!=-1?c.value:'' }&gradeId=${g.index}"
				aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
			</a></li>
			<li><a
				href="scoreServlet2?page=${1 }&name=${c.employee.name }&deptId=${c.employee.dept.id}&proId=${c.project.id}&value=${c.value!=-1?c.value:'' }&gradeId=${g.index}">首页</a></li>

			<c:forEach begin="${p.beginYe}" end="${p.endYe}" varStatus="status">


				<li <c:if test="${p.nowPage==status.index }">class="active" </c:if>><a
					href="scoreServlet2?page=${status.index }&name=${c.employee.name }&deptId=${c.employee.dept.id}&proId=${c.project.id}&value=${c.value!=-1?c.value:'' }&gradeId=${g.index}">${status.index }</a></li>
			</c:forEach>
			<li><a
				href="scoreServlet2?page=${p.pages}&name=${c.employee.name }&deptId=${c.employee.dept.id}&proId=${c.project.id}&value=${c.value!=-1?c.value:'' }&gradeId=${g.index}">尾页</a></li>
			<li><a id="next"
				href="scoreServlet2?page=${p.nowPage+1 }&name=${c.employee.name }&deptId=${c.employee.dept.id}&proId=${c.project.id}&value=${c.value!=-1?c.value:'' }&gradeId=${g.index}"
